 ...
 * @param {{ generate: Function, name: string, model: string }} provider - GitHub Models client
...
async function run() {
  try {
    ...
    if (!githubToken) {
      throw new Error(
        'GITHUB_TOKEN is required. Ensure `permissions: models: read` is set in the workflow.'
      );
    }

    const model = core.getInput('MODEL', { required: false }) || undefined;
    const filename = core.getInput('filename', { required: false });
    const baseBranch = core.getInput('branch', { required: false }) || 'main';

    const client = createGitHubModelsClient({ token: githubToken, model, logger });

    const { context } = github;
    const timestamp = new Date().toISOString().replaceAll(/[:.]/g, '-');
    const newBranch = `delegate/${timestamp}`;
    ...
    await runLLM(client, instructions, instructionFilePath);
    await createBranch(newBranch);
    await commitAndPush(
      `feat: delegate action changes\n\nGenerated by ${client.name} (${client.model}) as directed by @${context.actor}`,
      newBranch
    );
    ...
    await runLLM(client, reviewInstructions);
    await commitAndPush(
    ...
    const prNumber = await createPullRequest(
      ...
      `## Automated changes by Delegate Action\n\n` +
        ...
        promptFileLine +
        `**Model:** \`${client.model}\` (via GitHub Models)\n` +
        `**Base branch:** \`${baseBranch}\`\n` +
        `**Created by:** @${context.actor}\n\n` +
        `Please review the changes carefully before merging.\n\n` +
        `---\n\n` +
        `_Generated by GitHub Models as directed by @${context.actor}_`
    );

    if (prNumber) {
      await assignPR(githubToken, prNumber);
